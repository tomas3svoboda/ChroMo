...
def Lev1_Optim(experimentClustersComp, lossFunction, factor):
    ...
    res = minimize(Lev1_Loss_Function, gl.porosity, args=(experimentClustersComp, lossFunction, factor), bounds=Bounds(lb=gl.porosityRange[0], ub=gl.porosityRange[1]), method='Nelder-Mead',options={'fatol': 0.5})
    gl.porosity = res.x[0]
