...
from ChroMo_PE.functions.WebServerStuff.Serialize_File_To_JSON import Serialize_File_To_JSON
import ChroMo_PE.functions.solvers.Nonlin_Solver as _Nonlin_Solver_module
import ChroMo_PE.functions.solvers.CADET_Solver as _CADET_Solver_module
from ChroMo_PE.functions.Loss_Function_Analysis_Simple import Loss_Function_Analysis_Simple
from ChroMo_PE.functions.Model_Analysis import Model_Analysis
from ChroMo_PE.functions.solvers.Solver_Choice import Solver_Choice
...
def Web_Server():
    ...
    SOLVER_NONLIN_TYPE = 'PYTHON'
    SOLVER_MOLAR_MASS = 180.16
    SOLVER_CADET_NCOL = 10  # FVM cells for CADET — independent of spacialDiff (FDM grid points)

    def _resolve_solver(formInfo, isotherm):
        """Map isotherm type ('linear'/'nonlinear') + Solver Settings prefs to actual solver name and EDM backend.
        Returns (solver_name, nonlin_backend_or_None).
        """
        if isotherm == 'linear':
            return formInfo.get('linearSolver', 'Lin'), None
        else:
            ns = formInfo.get('nonlinearSolver', 'Nonlin_Python')
            if ns == 'CADET_Nonlin':
                return 'CADET_Nonlin', None
            backend = {'Nonlin_Python': 'PYTHON', 'Nonlin_CPP_FAST': 'CPP_FAST', 'Nonlin_CPP': 'CPP'}.get(ns, 'PYTHON')
            return 'Nonlin', backend
    ...
    class MainWorkThread(threading.Thread):
        ...
        def run(self):
            ...
            try:
                ...
                _Nonlin_Solver_module.nonlin_solver = getattr(_Nonlin_Solver_module.NonlinSolverType, formInfo.get("nonlinSolver", SOLVER_NONLIN_TYPE))
                _CADET_Solver_module.cadet_molar_mass = {
                    comp: formInfo.get(comp + "MolarMass", formInfo.get("cadetMolarMass", SOLVER_MOLAR_MASS))
                    for comp in compList.get(self.user_id, [])
                }
                _CADET_Solver_module.cadet_install_path = formInfo.get("cadetInstallPath") or None
                _CADET_Solver_module.cadet_ncol = int(formInfo.get("cadetNcol", SOLVER_CADET_NCOL))
                ...
                for comp in compList[self.user_id]:
                    ...
                    if formInfo["solver"] in ("Nonlin", "CADET_Nonlin"):
    ...
    class SimpleThread(threading.Thread):
        ...
        def run(self):
            ...
            import numpy as _np
            from ChroMo_PE.functions.solvers.Solver_Choice import Solver_Choice as _SC
            img_dir = BASE_FOLDER + '/functions/WebServerStuff/static/images/'

            def _save(fig):
                nonlocal plotFileCounter
                name = "plot" + str(plotFileCounter) + ".png"
                plotFileCounter += 1
                fig.savefig(img_dir + name)
                plt.close(fig)
                return name

            def _full_model_result(model):
                c = _np.asarray(model[0])
                t = _np.asarray(model[1])
                feed = _np.asarray(model[2]) if len(model) > 2 and model[2] is not None else _np.zeros(len(t))
                residuals = _np.asarray(model[3]) if len(model) > 3 and model[3] is not None else _np.zeros(len(t))
                mass_diff = model[4] if len(model) > 4 else 0.0
                if len(feed) != len(t):
                    feed = _np.resize(feed, len(t))
                if len(residuals) != len(t):
                    residuals = _np.resize(residuals, len(t))
                return c, t, feed, residuals, mass_diff

            try:
                plt.close('all')
                formInfo = formInfos[self.user_id]
                print("Manual Estimation graph thread started "
                      f"(user={self.user_id}, isotherm={formInfo.get('solver')}, "
                      f"nonlinear_solver={formInfo.get('nonlinSolver', SOLVER_NONLIN_TYPE)})",
                      flush=True)
                _Nonlin_Solver_module.nonlin_solver = getattr(
                    _Nonlin_Solver_module.NonlinSolverType,
                    formInfo.get("nonlinSolver", SOLVER_NONLIN_TYPE))
                comp2 = formInfo.get("comp2", "")
                _CADET_Solver_module.cadet_molar_mass = {
                    comp2: formInfo.get(comp2 + "MolarMass", formInfo.get("cadetMolarMass", SOLVER_MOLAR_MASS))
                }
                _CADET_Solver_module.cadet_install_path = formInfo.get("cadetInstallPath") or None
                _CADET_Solver_module.cadet_ncol = int(formInfo.get("cadetNcol", SOLVER_CADET_NCOL))
                ...
                params = [formInfo["porosity"], formInfo[formInfo["comp2"] + "K"],
                          formInfo[formInfo["comp2"] + "D"], formInfo["saturation"]]
                spacialDiff = formInfo["spacialDiff"]
                timeDiff    = formInfo["timeDiff"]
                time_       = formInfo["time"]
                solver      = formInfo["solver"]
                exp_idx     = formInfo["exp" + formInfo["comp2"]]
                comp        = formInfo["comp2"]

                # ── Plot 1: unpreprocessed outlet curve ──────────────────────────
                currExpSet2 = operator.Preprocess(experimentSet[self.user_id], False, False, False, 0)
                expCluster2 = operator.Cluster_By_Component(currExpSet2)
                expComp2    = expCluster2.clusters[comp][exp_idx]
                model2      = _SC(solver, params, expComp2, spacialDiff, timeDiff, time_, debugPrint=True)
                print("Manual Estimation unprocessed model solved.", flush=True)
                df2         = expComp2.concentrationTime
                fig1 = _mfig.Figure(figsize=(8, 5))
                ax   = fig1.add_subplot(111)
                ax.set_title("Experimental data")
                ax.set_xlabel("Time [s]"); ax.set_ylabel("Concentration [mg/mL]")
                ax.plot(model2[1], model2[0][:, -1])
                ax.scatter(df2.iloc[:, 0], df2.iloc[:, 1], color='r', marker=',', s=10)
                fname0 = _save(fig1)

                # ── Preprocess with user corrections ────────────────────────────
                currExpSet = operator.Preprocess(
                    experimentSet[self.user_id],
                    formInfo["gauss"], formInfo["retCorr"],
                    formInfo["massBal"], formInfo["retCorrThreshold"])
                if formInfo["retCorr"]:
                    formInfo["shifts"] = {os.path.split(e.metadata.path)[1]: e.shift
                                          for e in currExpSet.experiments}
                else:
                    formInfo["shifts"] = None
                if formInfo["massBal"]:
                    formInfo["originalFeedTimes"] = {os.path.split(e.metadata.path)[1]:
                        e.experimentCondition.originalFeedTime for e in currExpSet.experiments}
                    formInfo["newFeedTimes"]      = {os.path.split(e.metadata.path)[1]:
                        e.experimentCondition.feedTime      for e in currExpSet.experiments}
                else:
                    formInfo["originalFeedTimes"] = None
                    formInfo["newFeedTimes"]      = None

                expCluster  = operator.Cluster_By_Component(currExpSet)
                expComp     = expCluster.clusters[comp][exp_idx]
                model       = _SC(solver, params, expComp, spacialDiff, timeDiff, time_,
                                  debugPrint=True, full=True)
                print("Manual Estimation preprocessed model solved; creating plots.", flush=True)
                c, t, feed, residuals, massDiff = _full_model_result(model)
                df          = expComp.concentrationTime
                length      = expComp.experiment.experimentCondition.columnLength
                x           = _np.linspace(0, length, spacialDiff)
                x_plot      = _np.round(_np.linspace(0, spacialDiff - 1, 10)).astype(int)
                dx          = length / spacialDiff
                fnames = []

                # Fig 2: preprocessed outlet curve
                f = _mfig.Figure(figsize=(8, 5))
                ax = f.add_subplot(111)
                ax.set_title("Preprocessed data"); ax.set_xlabel("Time [s]"); ax.set_ylabel("Concentration [mg/mL]")
                ax.plot(t, c[:, -1]); ax.scatter(df.iloc[:, 0], df.iloc[:, 1], color='r', marker=',', s=10)
                fnames.append(_save(f))

                # Fig 3: 3-D surface
                f = _mfig.Figure(figsize=(8, 6))
                ax = f.add_subplot(111, projection='3d')
                X, Y = _np.meshgrid(x, t)
                ax.plot_surface(X, Y, c); ax.set_xlabel('Length [mm]'); ax.set_ylabel('Time [s]'); ax.set_zlabel('Concentration [mg/mL]')
                fnames.append(_save(f))

                # Fig 4: inlet concentration
                f = _mfig.Figure(figsize=(8, 5))
                ax = f.add_subplot(111)
                ax.set_title('Input concentration-time'); ax.plot(t, c[:, 0])
                fnames.append(_save(f))

                # Fig 5: residuals (zero for C++, actual for Python)
                f = _mfig.Figure(figsize=(8, 5))
                ax = f.add_subplot(111)
                ax.set_xlabel('Time [s]'); ax.set_ylabel('Residuals'); ax.plot(t, residuals)
                fnames.append(_save(f))

                # Fig 6: column positions
                f = _mfig.Figure(figsize=(9, 5))
                ax = f.add_subplot(111)
                ax.plot(t, feed, label='feed')
                for i in range(10):
                    ax.plot(t, c[:, x_plot[i]], label=f'{round(x_plot[i]*dx,0)} mm')
                ax.plot(t, c[:, -1], label=f'{round(spacialDiff*dx,0)} mm')
                ax.legend(fontsize=7)
                fnames.append(_save(f))

                self.progress = [fname0] + fnames
                print(f"Manual Estimation graph finished ({len(self.progress)} plot files).", flush=True)
            except Exception:
                _tb.print_exc()
                try:
                    with open(BASE_FOLDER + '/simplethread_error.log', 'w') as _f:
    ...
    def get_projects_test2():
        ...
        if not "nonlinSolver" in formInfos[flask_login.current_user.id]:
            formInfos[flask_login.current_user.id]["nonlinSolver"] = SOLVER_NONLIN_TYPE
        if not "cadetMolarMass" in formInfos[flask_login.current_user.id]:
            formInfos[flask_login.current_user.id]["cadetMolarMass"] = SOLVER_MOLAR_MASS
    ...
    def post_projects_test2():
        ...
        print("Manual Estimation graph requested.", flush=True)
        try:
            import traceback as _tb
            plt.clf()
            if not flask_login.current_user.id in formInfos:
            ...
            if not "nonlinSolver" in formInfos[flask_login.current_user.id]:
                formInfos[flask_login.current_user.id]["nonlinSolver"] = SOLVER_NONLIN_TYPE
            formInfo = formInfos[flask_login.current_user.id]
            print("Manual Estimation form data: " + str(request.form.to_dict()), flush=True)
            ...
            isotherm = str(request.form.get("solverTest"))
            actual_solver, nonlin_backend = _resolve_solver(formInfo, isotherm)
            formInfo["solver"] = actual_solver
            if nonlin_backend:
                formInfo["nonlinSolver"] = nonlin_backend
            ...
            exp_key = "expList" + formInfo["comp2"]
            formInfo["exp" + formInfo["comp2"]] = int(request.form.get(exp_key))
            ...
            if not thread_id in exporting_threads:
                print("Manual Estimation graph thread ID: " + str(thread_id), flush=True)
    ...
    def post_projects_test2_progress(id):
        ...
        try:
            formInfo = formInfos.get(flask_login.current_user.id, {})
            thread_id = int(id)
            if thread_id not in exporting_threads:
                return "<p style='color:red'>Session expired or server restarted. Please resubmit the form.</p>"
            progress = exporting_threads[thread_id].progress
            if progress == "-":
                return ""
            if not isinstance(progress, list):
                error_log = BASE_FOLDER + '/simplethread_error.log'
                if os.path.exists(error_log):
                    with open(error_log, 'r') as _f:
                        return "<p style='color:red'>Computation failed.</p><pre>" + _f.read() + "</pre>"
                return "<p style='color:red'>Computation failed - check server log for details.</p>"
            returnString = ""
            for idx, filename in enumerate(progress):
                ...
                if idx == 0:
                    returnString += render_template('GraphInfo.html', shifts=formInfo.get("shifts"),
                                    originalFeedTimes=formInfo.get("originalFeedTimes"),
                                    newFeedTimes=formInfo.get("newFeedTimes"))
            return returnString
        except Exception:
            err = _tb2.format_exc()
            print(err, flush=True)
            return "<p style='color:red'>Progress error:</p><pre>" + err + "</pre>", 500
    ...
    def post_projects_test2_continue():
        ...
        if formInfo["retCorr"]:
            formInfo["retCorrThreshold"] = float(request.form.get("retCorrThreshold"))
        formInfo["massBal"] = bool(request.form.get("massBalTest"))
        _iso = str(request.form.get("solverTest"))
        _sol, _nb = _resolve_solver(formInfo, _iso)
        formInfo["solver"] = _sol
        if _nb:
            formInfo["nonlinSolver"] = _nb
    ...
    def post_projects_params():
        ...
        isotherm = str(request.form.get("solver"))  # "linear" or "nonlinear" from the simplified dropdown
        actual_solver, nonlin_backend = _resolve_solver(formInfo, isotherm)
        formInfo["solver"] = actual_solver
        if nonlin_backend:
            formInfo["nonlinSolver"] = nonlin_backend
    ...
    def get_projects_result_list():
        nonlocal numberOfRunningOptims, db_mutex
        dbuser = flask_login.current_user.db
        # Collect IDs already linked to this user
        linked_ids = {res.thr_id for res in dbuser.results}
        # Scan all results in the DB and re-link any that aren't in the user's list
        orphaned = [r for r in DBResult.objects if r.thr_id not in linked_ids]
        if orphaned:
            with db_mutex:
                dbuser.reload()
                for r in orphaned:
                    dbuser.results.append(r)
                dbuser.save()
        resList = list(DBResult.objects.order_by('thr_id'))
    ...
    def get_projects_solver():
        nonlocal formInfos, experimentSet, clusterComp, compList
        if not flask_login.current_user.id in formInfos:
            formInfos[flask_login.current_user.id] = {}
        if not "spacialDiff" in formInfos[flask_login.current_user.id]:
            formInfos[flask_login.current_user.id]["spacialDiff"] = SOLVER_SPACIAL_DIFF
        ...
        if not "cadetMolarMass" in formInfos[flask_login.current_user.id]:
            formInfos[flask_login.current_user.id]["cadetMolarMass"] = SOLVER_MOLAR_MASS
        if not "solver" in formInfos[flask_login.current_user.id]:
            formInfos[flask_login.current_user.id]["solver"] = 'Lin'
        formInfo = formInfos[flask_login.current_user.id]
        # Load component list from experiments so molar-mass fields are populated
        page_compList = []
        try:
            if flask_login.current_user.id not in experimentSet:
                fetchExperimentData()
            if flask_login.current_user.id not in clusterComp:
                clusterComp[flask_login.current_user.id] = operator.Cluster_By_Component(
                    experimentSet[flask_login.current_user.id])
            compList[flask_login.current_user.id] = clusterComp[flask_login.current_user.id].clusters.keys()
            page_compList = list(compList[flask_login.current_user.id])
        except Exception:
            pass
        cpp_available = _Nonlin_Solver_module._cpp is not None
        return render_template('SolverSettings.html', formInfo=formInfo,
                               compList=page_compList, user=flask_login.current_user.id,
    ...
    def post_projects_solver():
        nonlocal formInfos
        formInfo = formInfos[flask_login.current_user.id]
        formInfo["linearSolver"] = str(request.form.get("linearSolver", "Lin"))
        formInfo["nonlinearSolver"] = str(request.form.get("nonlinearSolver", "Nonlin_Python"))
        ...
        # Derive legacy nonlinSolver (EDM C++ backend) for backward compat
        nl = formInfo["nonlinearSolver"]
        formInfo["nonlinSolver"] = {'Nonlin_Python': 'PYTHON', 'Nonlin_CPP_FAST': 'CPP_FAST',
                                    'Nonlin_CPP': 'CPP'}.get(nl, 'PYTHON')
        # CADET install path (empty string → auto-detect)
        install_path = request.form.get("cadetInstallPath", "").strip()
        formInfo["cadetInstallPath"] = install_path if install_path else None
        # CADET spatial resolution (FVM cells, independent of spacialDiff)
        ncol_val = request.form.get("cadetNcol", "").strip()
        formInfo["cadetNcol"] = int(ncol_val) if ncol_val else SOLVER_CADET_NCOL
        # Per-component CADET molar masses
        for key, value in request.form.items():
            if key.endswith("MolarMass") and value.strip():
                try:
                    formInfo[key] = float(value)
                except ValueError:
                    pass
    ...
    def get_main_page():
        ...
        for dbuser in DBUser.objects:
            ...
            if username in dbuser.username and valid_password:
                user = User()
                user.id = username
                flask_login.login_user(user)
                if not user.id in formInfos:
                    formInfos[user.id] = {}
                os.makedirs(UPLOAD_FOLDER + "/" + user.id, exist_ok=True)
                return redirect(url_for('upload_file_page'))
    ...
    def handle_unhandled_exception(e):
        ...
        err = _tb_global.format_exc()
        print("UNHANDLED EXCEPTION:\n" + err, flush=True)
        return "<p style='color:red'>Server error (unhandled exception):</p><pre>" + err + "</pre>", 500

    #api.run(debug=True)
    print("localhost:6969")
    serve(api, listen='*:6969', threads=16)
