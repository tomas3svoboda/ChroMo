...
function formSolverChange(){
    ...
    const isNonlin = (solver == "nonlinear")
    const isLin = (solver == "linear")
...
function formSolverChangeForTest(){
    ...
    if(isNonlin){
        div.innerHTML =`
                ...
                <input type="number" id="saturationTest" name="saturationTest" min="0" step="any" required><br>
           `
        let params = document.getElementById("paramsTest")
        if(params) params.innerHTML = params.innerHTML.replaceAll("Henry","Langmuir")
    }
    else if(isLin){
        div.innerHTML = ""
        let params = document.getElementById("paramsTest")
        if(params) params.innerHTML = params.innerHTML.replaceAll("Langmuir","Henry")
    }
...
async function showGraph2(f = "mainform", aurl = "", i = "imgdiv", showEl = ""){
    ...
    try{
        ...
        output.innerHTML = "<p>Starting graph calculation...</p>"
        console.log("Manual Estimation graph POST started")
        ...
        if(!resp.ok){
            if(resp.status === 401){
                output.innerHTML = "<p style='color:red'>Session expired. Please <a href='/'>log in again</a>.</p>"
            } else {
                output.innerHTML = "<p style='color:red'>Graph request failed: " + resp.status + "</p>" + path
            }
            return false
        }
        ...
        output.innerHTML = "<p>Calculation running. Waiting for plots...</p>"
        while(1){
            await sleep(500)
            let resp2 = await fetch(path)
            let content = await resp2.text()
            if(!resp2.ok){
                output.innerHTML = "<p style='color:red'>Graph progress failed: " + resp2.status + " " + content + "</p>"
                return false
            }
            if(content != ""){
        ...
        output.innerHTML = "<p style='color:red'>Graph request failed: " + error + "</p>"
        console.error(error)
        return false
    }
    return false
