...
def prepare_matrices(Nx, dt, dx_dense, dx_sparse, flowSpeed, disperCoef, henryConst, porosity, Nx_dense):
    ...
    Returns:
    Matrices A and B used in the Crank-Nicolson numerical scheme.
    """
...
#=========================================================================================================================
#=========================================================================================================================
def discretize_and_solve(Nx, Nt, dx_dense, dx_sparse, dt_dense, dt_sparse,
                         disperCoef, flowSpeed, henryConst, porosity, feed,
                         c, debugPrint, Nx_dense, denseSparseRatio):
    """
    ...
    A, B = prepare_matrices(Nx, dt_dense, dx_dense, dx_sparse, flowSpeed, disperCoef,
                            henryConst, porosity, Nx_dense)
    for i in range(1, Nt):
        if debugPrint:
            if i == 1:
        ...
        if i == 1 or dt_current != (dt_dense if i-1 <= (Nt * denseSparseRatio) else dt_sparse):
            A, B = prepare_matrices(Nx, dt_current, dx_dense, dx_sparse, flowSpeed,
                                    disperCoef, henryConst, porosity, Nx_dense)
...
def data_struct_prep(length, Nx, Nt, feedTime, feedConc, time):
    ...
    feed = normalize_feed_pulse(feed, t, feedTime, feedConc)
