...
'''
def Nonlin_Solver(
                  flowRate = 1000,       # Volume flowrate in [mL/h]
                  length = 2000,         # Lenght of the packed section in the column [mm]
                  diameter = 25,        # Column diameter [mm]
                  feedVol = 40,         # Feed injection volume [mL]
                  feedConc = 180.81,    # Concentration of the balanced component in the feed [g/mL] or [mg/mm^3]
                  porosity = 0.376,       # Total porosity of the sorbent packing [-]
                  langmuirConst = 0.14,  # Langmuir's constant of the linear isotherm [-]
                  disperCoef = 409.527,       # Axial dispersion coefficient [mm^2/s]
                  saturCoef = 47.558,       # Saturation Coefficient
    ...
    if debugPrint:
        ...
        print("\nCourant-Friedrichs Numbers Matrix:")
    ...
    # Normalise: scale so time-integral of feed equals feedTime * feedConc exactly,
    # regardless of how many ramp steps there are or what dt_dense happens to be.
    injected_area = np.trapz(feed, t)
    if abs(injected_area) > 1e-15:
        feed *= (feedTime * feedConc) / injected_area
        ...
        for i in range(0, len(c0)):  # Main loop through all the vector's elements
            ...
            if i == 0:  # Left boundary
                ...
                denominator0 = 1 + (((1 - porosity) * saturCoef * langmuirConst) / \
                               ((((langmuirConst * c0[i] + 1) ** 2) * porosity)))
                denominator1 = 1 + (((1 - porosity) * saturCoef * langmuirConst) / \
                               ((((langmuirConst * c1[i] + 1) ** 2) * porosity)))
...
res = Nonlin_Solver(full=True, debugPrint=True, debugGraph=True)
#res = Nonlin_Solver(full=False, debugPrint=False, debugGraph=False)
...
#==========================================================================
""" Save column output into excel file"""
# Extract c, t, and feed
c = res[0]  # Concentrations (2D array)
t = [(x / 60) for x in res[1]]
feed = res[2]  # Feed (1D array)

# Extract the last column of c
c_last = c[:, -1]

# Create a DataFrame with Time, Concentration (last column), and Feed
df = pd.DataFrame({
    'Time': t,
    'Concentration_Last': c_last,
    'Feed': feed
})

# Save to Excel
output_file = "solver_fit150only_longFeed.xlsx"
