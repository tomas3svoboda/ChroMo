...
from objectiveSMB_linear import objectiveSMB_linear

def parameterEstimationSMB_linear(
    experimental_data_path,
    end_time,
    initial_guess,          # [K_A, K_B, delta_shared]
    bounds,                 # list of 3 (low, high)
    flow_rates=None,
    switch_interval=780,
    dt=1, Nx=30,
    ...
    """
    Fit Linear EDM params: K_A, K_B, delta_shared.
    ...
    if eval_log_csv:
        with open(eval_log_csv, "w", encoding="utf-8") as f:
            f.write("eval_idx,K_A,K_B,delta_shared,MSE\n")

    eval_idx = [0]
    def obj(p):
        K_A, K_B, delta_shared = p
        mse = objectiveSMB_linear(
            end_time=end_time,
            experimental_data_path=experimental_data_path,
            name_compA=name_compA, feed_concA=feed_concA,
            name_compB=name_compB, feed_concB=feed_concB,
            K_A=K_A, K_B=K_B, delta_shared=delta_shared,
        ...
        if print_eval:
            print(f"[eval {eval_idx[0]:04d}] MSE={mse:.6g} | "
                  f"K_A={K_A:.6g}, K_B={K_B:.6g}, δ={delta_shared:.6g}")
        if eval_log_csv:
            with open(eval_log_csv, "a", encoding="utf-8") as f:
                f.write(f"{eval_idx[0]},{K_A},{K_B},{delta_shared},{mse}\n")
        return mse
    ...
    def _callback(xk):
        ...
        print(f"[iter {it_idx[0]:03d}] best_so_far: "
              f"K_A={xk[0]:.6g}, K_B={xk[1]:.6g}, δ={xk[2]:.6g}")

    res = minimize(
        obj, x0=initial_guess, method="L-BFGS-B", bounds=bounds,
        options={
            "maxiter": 5000,
            "ftol": 1e-6,     # stop when improvement is tiny
    ...
    '''res = minimize(
        ...
        method="Powell",
        options={
            "maxiter": 5000,   # max počet iterací
            "maxfev": 50000,   # max počet vyhodnocení funkce
            "xatol": 1e-4,    # tolerance na změnu v x
            "fatol": 1e-6,    # tolerance na změnu v f(x)
            "adaptive": True  # škáluje simplex (užitečné pro různé měřítko param.)
        },
        callback=_callback
    )'''
...
if __name__ == "__main__":
    ...
    bounds = [(0.0001, 32.26), (0.0001, 32.26), (1.0, 813000.0)]
