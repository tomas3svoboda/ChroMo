...
class Tube:
    ...
    def step(self, cins):
        ...
        Returns per-component axial profiles (like before).
        """
        outputs = []
        for comp, cin in zip(self.components, cins):
            c = comp.c
            if BYPASS_TUBE_DELAY:
                # diagnostic: ideal pipe (no delay); do NOT use in production
                c[:] = cin
                outputs.append(c.tolist())
                continue

            alpha = float(self._alpha)
            # number of bounded substeps in this dt
            nsub = int(np.ceil(max(1.0, alpha)))
            a = alpha / nsub  # <= 1

            # per-substep upwind update:
            # c <- (1-a)*c + a*roll(c,1); enforce inlet at index 0
            for _ in range(nsub):
                c = (1.0 - a) * c + a * np.roll(c, 1)
                c[0] = cin
            comp.c = c
            outputs.append(c.tolist())
        return outputs
